...
// .wrangler/tmp/bundle-7TPbwL/checked-fetch.js
...
// .wrangler/tmp/bundle-7TPbwL/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-7TPbwL/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-7TPbwL/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
function calculateNewMastery(p_know, is_correct, config = DEFAULT_CONFIG) {
  let p_posterior = 0;
  if (is_correct) {
  ...
  return Math.min(0.9999, Math.max(1e-4, p_next));
}
__name(calculateNewMastery, "calculateNewMastery");
...
function calculateSimilarity(target, actual) {
  const normalize = /* @__PURE__ */ __name((s3) => s3.toLowerCase().replace(/[^\w\s]/g, "").trim(), "normalize");
...
app.use("/*", cors());
app.get("/", (c2) => {
  return c2.text("Hola, Espa\xF1ol Pro API!");
});
app.post("/api/upload/presign", async (c2) => {
  try {
    const { filename, contentType } = await c2.req.json();
    if (!filename)
      return c2.json({ error: "Filename is required" }, 400);
    const S3 = new S3Client({
      region: "auto",
      endpoint: `https://${c2.env.ACCOUNT_ID}.r2.cloudflarestorage.com`,
      credentials: {
    ...
    return c2.json({ uploadUrl, key });
  } catch (e2) {
    console.error("Presign error:", e2);
    return c2.json({ error: "Failed to generate upload URL" }, 500);
  }
});
app.get("/api/courses", async (c2) => {
  const userId = c2.req.query("userId") || "test_user_1";
  try {
    ...
    return c2.json({ error: e2 }, 500);
  }
});
app.get("/api/courses/:id", async (c2) => {
  const id = c2.req.param("id");
  const userId = c2.req.query("userId") || "test_user_1";
  try {
    const course = await c2.env.DB.prepare(
      "SELECT * FROM courses WHERE id = ?"
    ).bind(id).first();
    if (!course)
      return c2.json({ error: "Course not found" }, 404);
    const { results: units } = await c2.env.DB.prepare(
      "SELECT * FROM units WHERE course_id = ? ORDER BY order_index"
    ).bind(id).all();
    const { results: completedLessons } = await c2.env.DB.prepare(`
    ...
    `).bind(userId, id).all();
    const completedLessonIds = completedLessons.map((r2) => r2.lesson_id);
    return c2.json({ ...course, units, completedLessonIds });
  } catch (e2) {
    return c2.json({ error: e2 }, 500);
  }
});
app.get("/api/units/:id/lessons", async (c2) => {
  const id = c2.req.param("id");
  try {
    const { results } = await c2.env.DB.prepare(
...
app.get("/api/lessons/:id", async (c2) => {
  const id = c2.req.param("id");
  try {
    const lesson = await c2.env.DB.prepare(
      "SELECT * FROM lessons WHERE id = ?"
    ).bind(id).first();
    if (!lesson)
      return c2.json({ error: "Lesson not found" }, 404);
    return c2.json(lesson);
  } catch (e2) {
    return c2.json({ error: e2 }, 500);
  }
});
app.post("/api/progress", async (c2) => {
  try {
    ...
    ).bind(userId, lessonId, interactionType || "COMPLETION", isCorrect ? 1 : 0, Math.floor(Date.now() / 1e3)).run();
    ...
    if (progress) {
      ...
      ).bind(Math.floor(Date.now() / 1e3), userId, courseId).run();
    } else {
      await c2.env.DB.prepare(
        "INSERT INTO user_course_progress (user_id, course_id, completed_lessons_count, last_updated) VALUES (?, ?, 1, ?)"
      ).bind(userId, courseId, Math.floor(Date.now() / 1e3)).run();
    }
    const lesson = await c2.env.DB.prepare("SELECT kc_id FROM lessons WHERE id = ?").bind(lessonId).first();
    if (lesson && lesson.kc_id) {
      const kcId = lesson.kc_id;
      const kcState = await c2.env.DB.prepare(
        "SELECT p_know FROM user_kc_state WHERE user_id = ? AND kc_id = ?"
      ).bind(userId, kcId).first();
      const currentP = kcState ? kcState.p_know : 0.01;
      ...
      if (kcState) {
        ...
        ).bind(newP, Math.floor(Date.now() / 1e3), userId, kcId).run();
      } else {
        await c2.env.DB.prepare(
          "INSERT INTO user_kc_state (user_id, kc_id, p_know, last_practice_time) VALUES (?, ?, ?, ?)"
        ).bind(userId, kcId, newP, Math.floor(Date.now() / 1e3)).run();
      }
    }
    return c2.json({ success: true });
  } catch (e2) {
    console.error(e2);
    return c2.json({ error: "Failed to record progress" }, 500);
  }
});
app.post("/api/auth/login", async (c2) => {
  try {
    const { email } = await c2.req.json();
    if (!email)
      return c2.json({ error: "Email is required" }, 400);
    ...
    if (user) {
      ...
      ).bind(newId, email, displayName, Math.floor(Date.now() / 1e3)).run();
      return c2.json({ id: newId, email, display_name: displayName });
    }
  } catch (e2) {
    console.error(e2);
...
app.post("/api/ai/evaluate-speech", async (c2) => {
  try {
    ...
    if (contentType.includes("application/json")) {
      ...
      if (fileKey) {
        const object = await c2.env.BUCKET.get(fileKey);
        if (!object) {
          return c2.json({ error: "Audio file not found in storage" }, 404);
        }
    ...
    if (!audioBuffer) {
      return c2.json({ error: "Audio input is required (file or fileKey)" }, 400);
    }
    if (!referenceText) {
      return c2.json({ error: "Reference text is required" }, 400);
    }
    const aiResponse = await c2.env.AI.run("@cf/openai/whisper", {
    ...
    console.error(e2);
    return c2.json({ error: "AI evaluation failed" }, 500);
  }
});
app.post("/api/ai/chat", async (c2) => {
  try {
    const { messages } = await c2.req.json();
    if (!messages || !Array.isArray(messages)) {
      return c2.json({ error: "Messages array is required" }, 400);
    }
    ...
    if (!processedMessages.some((m2) => m2.role === "system")) {
      processedMessages.unshift({
        role: "system",
        content: "You are a helpful Spanish tutor." + grammarInstruction
      });
    }
    const aiResponse = await c2.env.AI.run("@cf/meta/llama-3-8b-instruct", {
      messages: processedMessages
    });
    return c2.json({ response: aiResponse.response });
  } catch (e2) {
    console.error(e2);
    return c2.json({ error: "AI Chat failed" }, 500);
  }
});
app.get("/:filename", async (c2) => {
  const filename = c2.req.param("filename");
  const object = await c2.env.BUCKET.get(filename);
  if (object === null) {
    return c2.text("Object Not Found", 404);
  }
...
app.get("/api/users/:userId/skills", async (c2) => {
  ...
  try {
    ...
    return c2.json({ error: "Failed to fetch skills" }, 500);
  }
});
var src_default = app;
